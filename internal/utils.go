...
func TimesStampISO() string {
	currentTime := time.Now()
	isoFormat := currentTime.UTC().Format("2006-01-02T15:04:05.999Z")
	return isoFormat
}
...
func Headers(request *http.Request, apikey string, timestamp string, sign string, passphrase string) {
	...
	request.Header.Add(constants.OkxAccessKey, apikey)
	request.Header.Add(constants.OkxAccessSign, sign)
	request.Header.Add(constants.OkxAccessTimestamp, timestamp)
	request.Header.Add(constants.OkxAccessPassphrase, passphrase)
}
